-- Fix room_id type mismatch in Zoho integration
-- This addresses the foreign key constraint error for incompatible types
...
-- If zoho_channel_mappings table exists, drop it and recreate with correct types
DROP TABLE IF EXISTS zoho_sync_jobs CASCADE;
DROP TABLE IF EXISTS zoho_channel_mappings CASCADE;

-- Recreate zoho_channel_mappings with correct room_id type (integer)
...
-- Recreate sync job tracking table
...
SELECT 'Zoho integration room_id type mismatch fixed' as status;
